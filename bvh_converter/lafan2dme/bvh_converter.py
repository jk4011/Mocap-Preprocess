...
class BvhConverter(object):
    """
    class supports deletion, insertion, renaming, and saving file.
    """
    ...
    def delete_joint(self, joint, pass_pos=True):
        # find idx of joint
    ...
    def insert_child_joint(self, parent, joint, idx=None, loc="middle"):
        """tree에서 leaf를 insert할 때 사용합니다."""
        # find where to insert
        parent_idx = self.bones.index(parent)
        if idx==None:
            idx = parent_idx + 1
        ...
        self.parents = np.insert(self.parents, idx, self.parents[idx-1])
